const container = document.querySelector('#translateMenuContainer');
...
(function (w, d, u) {
    ...
    function encodeAllSpecialTags(html) {
        ...
        html = html.replace(/<b>/gim, "&lt;b&gt;").replace(/<\/b>/gim, "&lt;/b&gt;").replace(/<i>/gim, "&lt;i&gt;").replace(/<\/i>/gim, "&lt;/i&gt;").replace(/<u>/gim, "&lt;u&gt;").replace(/<\/u>/gim, "&lt;/u&gt;").replace(/<em>/gim, "&lt;em&gt;").replace(/<\/em>/gim, "&lt;/em&gt;").replace(/<\/strong>/gim, "&lt;/strong&gt;").replace(/<\/abbr>/gim, "&lt;/abbr&gt;").replace(/<\/sub>/gim, "&lt;/sub&gt;").replace(/<\/sup>/gim, "&lt;/sup&gt;").replace(/<\/big>/gim, "&lt;/big&gt;").replace(/<\/small>/gim, "&lt;/small&gt;".replace(/<br>/gim, "&lt;br&gt;").replace(/<\/br>/gim, "&lt;/br&gt;"));
    ...
    function translateDOM() {
        ...
        if (translationJSON) {
            ...
            langMenu.id = "langswitcher";
            langMenu.className = "notranslate";
            var customLangMenu = d.createElement("span");
            var customLangMenuHtml = translationJSON.custom_lang_switcher_html;

            var languages = translationJSON.languages;    
            if (languages.length > 1) {
                ...
                for (var i = 0; i < languages.length; i++) {
                    if (languages[i].lang_code != currlangcode) {
                        ...
                        if (translationJSON.custom_lang_switcher_html && translationJSON.custom_lang_switcher_container_id) {
                        ...
                        if (translationJSON.custom_lang_switcher_html && translationJSON.custom_lang_switcher_container_id) {
                            ...
                            currlangItem.id = "currentlanglink";
                            currlangItem.href = "";
                ...
                //         d.getElementById('langswitcher').style.height = menuHeight + 'px';
                //     });
                //     langMenu.addEventListener("mouseout", function (event) {
                //         setTimeout(function () { d.getElementById('langswitcher').style.height = '30px'; }, 1000)
                //     });
                // } else {
                ...
                //     });
                // }
                var customMenuContainer = d.getElementById(translationJSON.custom_lang_switcher_container_id);
                if (!translationJSON.translated_pages) {
                    isTranslated = true;
                    container.insertBefore(langMenu, container.firstChild);

                    if (translationJSON.custom_lang_switcher_html && translationJSON.custom_lang_switcher_container_id && customMenuContainer) {
                    ...
                    for (var i = 0; i < translationJSON.translated_pages.length; i++) {
                        var comparableLink = createComparableLink(w.location.href, languages);
                        if (comparableLink.replace(currlangcode, '').replace('//', '/') == translationJSON.translated_pages[i].slug.toLowerCase()) {
                            isTranslated = true;
                            break;
                        }
                    }
            ...
            if (translationJSON.translated_segments && translationJSON.translated_segments.length > 0 && isTranslated && currlangcode != sourceLang.lang_code) {
                var translatedSegments = translationJSON.translated_segments;

                var specialTags = getElementsByTagNames(['b', 'u', 'i', 'strong', 'em', 'abbr', 'sub', 'sup', 'big', 'small', 'br']);
                    ...
                    if (link.attributes["href"] && link.parentNode.className != 'notranslate') {
                        ...
                        if (link.attributes["href"].value.indexOf(parseURL(w.location.href, true)) != -1 || link.attributes["href"].value.indexOf(sourceLang.url_pattern) != -1 || !link.attributes["href"].value.toLowerCase().startsWith('http')) {
                            ...
                            if (!translationJSON.translated_pages) {
                                ...
                                for (var j = 0; j < translationJSON.translated_pages.length; j++) {
                                    if (comparableLink == translationJSON.translated_pages[j].slug.toLowerCase()) {
                                        rewriteUrl(link, absoluteLink, translationLang, url_pattern);
                                        break;
                                    }
                                }
            ...
            if (translationJSON.translated_image_segments && translationJSON.translated_image_segments.length > 0 && isTranslated && currlangcode != sourceLang.lang_code) {
                var translatedImageSegments = translationJSON.translated_image_segments;
    ...
    w.addEventListener ? w.addEventListener("load", translationLoader, false) : w.attachEvent("onload", translationLoader);
}(window, document));